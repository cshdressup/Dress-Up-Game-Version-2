...
let selectedShirt = 0, selectedPantsImg = 0, selectedPantsCol = 1, selectedShoes = 0, selectedHat = -1;         
...
function preload() {
  ...
  for (let i = 0; i <= 3; i++) faces.push(loadImage('assets/face' + i + '.png'));
  for (let i = 0; i <= 7; i++) hairs.push(loadImage('assets/hair' + i + '.png'));
...
function drawFinal() {
  ...
  if (selectedHat >= 0 && hats[selectedHat]) image(hats[selectedHat], 0, 0, IMG_W, IMG_H);
  pop();
...
function resetGame() {
  ...
  selectedShirt = 0; selectedPantsImg = 0; selectedPantsCol = 1; selectedShoes = 0; selectedHat = -1;
  confetti = []; 
}
